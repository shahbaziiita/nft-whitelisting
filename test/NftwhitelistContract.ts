...
import { NftWhitelistContract } from "../typechain-types";
...
describe("Merkle tree whitelist", function () {
  ...
  beforeEach(async function () {
    ...
    this.contract = <NftWhitelistContract>await ContractFactory.deploy();
  ...
  it("Should revert INVALID_PROOF", async function () {
    ...
    await expect(
      this.contract.connect(this.signers.all[4]).uri(proof, 32)
  ...
  it("Should pass whitelist check", async function () {
    ...
    await expect(
      this.contract.connect(this.signers.all[1]).uri(proof, 32)
    ).to.be.not.reverted;
  });
});